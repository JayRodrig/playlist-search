...
const render = () => {
  ...
  const songs_list = document.querySelector('.song-list');
  let combinedHTML = '';
  ...
  songs_list.innerHTML = combinedHTML;
}
...
const search = () => {
  ...
  for(let i = 0; i < playlist.songs.length; i++){
    ...
    if(songName.toLowerCase().includes(input.value.toLowerCase())){
      result.push(song);
    }
  }
  for(let j = 0; j < result.length; j++){
    const song = result[j];
    combinedHTML += objectToHTML(song);
  }
  songs_list.innerHTML = combinedHTML;
}
